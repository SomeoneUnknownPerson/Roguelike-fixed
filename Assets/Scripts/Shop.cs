...
public class Shop : Singleton<Shop>
{
    ...
    public void SelectObject()
    {
        if (currentID == -1)
        {
            ...
            if(DataBase.Instance.items[items[currentID].id].cost > Inventory.Instance.items[0].count)
            {
                Debug.Log("d");
                isAdded = false;
            }
            ...
            else if (items[currentID].type != DataBase.ItemType.Empty)
            {
                for (int i = 5; i < Inventory.Instance.maxCount; i++)
                {
                    ...
                    {
                        if (Inventory.Instance.items[i].count + 1 <= 128)
                        {
                            Inventory.Instance.AddItem(i, DataBase.Instance.items[items[currentID].id], Inventory.Instance.items[i].count + 1, items[currentID].type);
            ...
            if (!isAdded && DataBase.Instance.items[items[currentID].id].cost <= Inventory.Instance.items[0].count)
            {
                for (int i = 5; i < Inventory.Instance.maxCount; i++)
                {
                    if (Inventory.Instance.items[i].id == 0)
                    {
                        Inventory.Instance.AddItem(i, DataBase.Instance.items[items[currentID].id], 1, items[currentID].type);
                        AudioManager.Instance.PlayEffects(goldSound);
                        Inventory.Instance.items[0].count -= DataBase.Instance.items[items[currentID].id].cost;
                        isAdded = true;
                        break;
